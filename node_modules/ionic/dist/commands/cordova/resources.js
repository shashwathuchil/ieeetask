...
This command uses Ionic servers, so we require you to be logged into your free Ionic account. Use ${chalk_1.default.green('ionic login')} to login.
    ...
    run(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            let platforms = yield getPlatforms(integration.root);
            ...
            if (platform && !platforms.includes(platform)) {
                ...
                if (confirm) {
                    yield installPlatform(this.env, platform, integration.root);
                    yield conf.reload();
                    platforms = yield getPlatforms(integration.root);
                    debug(`platforms=${platforms.map(e => chalk_1.default.bold(e)).join(', ')}`);
                }
                else {
            ...
            if (!force) {
                ...
                if (imgResources.length === 0) {
                    this.env.tasks.end();
                    this.env.log.nl();
                    this.env.log.ok('No need to regenerate images.\n' +
